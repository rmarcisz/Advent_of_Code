...
class AdventOfCodeD15:
    ...
    def _move_robot_2(self, direction):
        to_check = [tuple(np.sum([self.maze_map['@'], self.directions[direction]], axis=0))]
        to_move = [tuple(np.sum([self.maze_map['@'], self.directions[direction]], axis=0))]
        while to_check != []:
            if to_check[0] in self.maze_map['#']:
                return
            elif to_check[0] in self.maze_map['[']:
                if direction in ['^', 'v']:
                    to_check.append(tuple(np.sum([to_check[0], self.directions[direction]], axis=0)))
                    to_check.append(tuple(np.sum([to_check[0], self.directions[direction], (0,1)], axis=0)))
                    to_move.append(tuple(np.sum([to_check[0], self.directions[direction]], axis=0)))
                    to_move.append(tuple(np.sum([to_check[0], self.directions[direction], (0,1)], axis=0)))
                else:
                    to_check.append(tuple(np.sum([to_check[0], self.directions[direction], (1,0)], axis=0)))
